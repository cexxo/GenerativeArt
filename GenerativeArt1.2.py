...
def randomness(nLayers, nElements,randoms,fields,d,collectible_metadata):
    ...
    for k in range(1,nLayers):                                              # generate the random numbers for choosing the element i want in
        for j in range(1,nElements[k]):
            temp_list.append(j)
            temp_weight.append(j*j)
        temp_rand = random.choices(temp_list,temp_weight)                      # the NFT. 
        randoms.append(temp_rand[0])                                       #The metadata file is gonna be updated with the corresponing value
        temp_meta = get_meta(fields,k)                                  #of the element we chose for our layer.
        list = d[k-1]
        collectible_metadata[temp_meta] = list[temp_rand[0]]               #the metadata field is gonna be updated with the value of the 
                                                                        #chosen field.
        temp_list.clear()
        temp_weight.clear()
...
"""def main():
    ...
    count = 0
    while(True):
        a = random.choices(randoms, weights=[11,11,11,11,11,11,11,11,11,0.5])
        count = count + 1
        print(a)
        if (a[0] == 10):
            break
