...
def main():
    ...
    try:
        nLayers = int(my_file.readline())                                   #the first value that you read is the number of layers in the NFT
    except:
        print("the first line of pahts.txt is not a number...")             #error message in case the first line of the file is not a number
    try:
        nElements = int(my_file.readline())                                 #the second value is the number of elements that our layer has avaiable
    except:
        print("the second line of paths.txt is non a number...")            #error message in case the second line of the file is not a number
    try:
    ...
    if num_NFT > nElements**nLayers:                                        #i check that the number of NFTs required is lower than all the
        num_NFT = nElements**nLayers                                        #possible combinations, if not i notify it to the user
    ...
    for i in range(num_NFT):                                                #this is the whole cycle that generates our NFTs
        ...
        baseImage.save("result{}.png".format(i+1),"PNG")                    #I save the image as the i-th result of our production process.
